...
def dlPredict():
    try:
        ...
        for root, dirs, files in os.walk(filePath):
            logger.debug('Files Available:')
            logger.debug(files)
            for file in files:
                if file.endswith(".yaml"):
                    kerasyamlFile = os.path.join(directory, modelName, file);
                    keras_yaml = None
                    try:
                        keras_yaml = models_cache[kerasyamlFile]
                    except KeyError as e:
                        with open(os.path.join(directory, modelName, file), "rb") as yaml_file:
                            keras_yaml = yaml_file.read()
                            models_cache[kerasyamlFile] = keras_yaml

                if file.endswith(".h5"):
                    model = model_from_yaml(keras_yaml)
                    model.load_weights(os.path.join(directory, modelName, file))

                # loading keras tokenizer
                if file.endswith(".pkl") and fnmatch.fnmatch(file, '*tok*.pkl'):
                    tokenizerFile = os.path.join(directory, modelName, file);
                    tokenizer = None
                    try:
                        tokenizer = models_cache[tokenizerFile]
                    except KeyError as e:
                        with open(os.path.join(directory, modelName, file), "rb") as f:
                            tokenizer = pickle.load(f)
                            models_cache[tokenizerFile] = tokenizer

                # loading the enocder
                if file.endswith(".pkl") and fnmatch.fnmatch(file, '*enc*.pkl'):
                    encoderFile = os.path.join(directory, modelName, file);
                    encoder = None
                    try:
                        encoder = models_cache[encoderFile]
                    except KeyError as e:
                        with open(os.path.join(directory, modelName, file), "rb") as f:
                            encoder = pickle.load(f)
                            models_cache[encoderFile] = encoder
        ...
        X_test = tokenizer.texts_to_matrix(testdataL, mode='count')
        text_labels = encoder.classes_

        ################## Prediction################
        prediction = model.predict(np.array(X_test))
        prediction = text_labels[np.argmax(prediction)]

        ##################confidence score################
        confidenceScore = model.predict_proba(np.array(X_test))
        confidenceScore = confidenceScore[0][np.argmax(confidenceScore)]

        ################## Data Frame Conversion ##############
        prediction = pd.DataFrame(pd.Series(prediction))
        ...
        logger.debug('Categories in the Data Set')
        logger.debug(text_labels)
...
##########################################################################################
#Keras with backend as "TensorFlow" - Without Authentication
##########################################################################################
...
class PredictClass(Resource):
    ...
    def post(self):
        print("no auth")
        testdata,prediction,confScrore =dlPredict()
        return jsonify(text=testdata,
                       topic={'name': prediction['label'][0],
                              'score': confScrore})

##########################################################################################
#Keras with backend as "TensorFlow" - With Authentication
##########################################################################################
